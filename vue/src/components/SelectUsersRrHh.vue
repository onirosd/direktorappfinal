    ...
    <div id="listarusuarios" class="flex mb-6 flex-col w-full overflow-auto max-h-[300px]">
        <div v-for="(user, index) in filteredUsers" :key="index" class="flex mb-4">
            <input
                ...
                class="w-6 h-6 border border-[#8A9CC9] rounded mr-4 accent-orange"
                v-model="this.$store.state.rrhh_rows[rowIndex].integrantes"
  ...
  export default {
    ...
    methods: {
        capturamosCambios: function(payload) {
          ...
          let data_update = this.$store.state.rrhh_rows[this.rowIndex].integrantes
          this.$emit('capturamosCambios', {lista: data_update, codProyecto: this.$store.state.rrhh_rows[this.rowIndex].codProyecto});

        },
        updSelectAll: function(){

          this.selAllState = this.$store.state.rrhh_rows[this.rowIndex].integrantes.length != this.$store.state.rrhh_rows[this.rowIndex].integrantesProy.length ? false : true
        ...
        selectAll: function() {
            ...
            if(this.selAllState === false) {
                ...
                this.$store.state.rrhh_rows[this.rowIndex].integrantes = []
            }
            else {

              let pintegrantes = this.$store.state.rrhh_rows[this.rowIndex].integrantesProy
              let integrantes  = []
              ...
              this.$store.state.rrhh_rows[this.rowIndex].integrantes = integrantes;
