<template>
    ...
    <div v-if="isLoading">
      <Breadcrumb
        :paths="['Inicio', 'Modulo de RRHH']"
        :urls ="['home']"
        :settingFlag="false"
      />
      <Indicator
        :header="'Módulo de RRHH'"
        :paragraph="'Acá podrás visualizarel flujo de personal de tus proyectos y entrar al detalle de cada uno'"
  ...
  import SelectUsers from "../../components/SelectUsersRrHh.vue";
  ...
  export default {
    ...
    methods: {
      ...
      capturamosCambios: function(data) {
        ...
        store.dispatch('update_rrhh_member', data).then(res => {
      ...
      editStatusSave: function(payload) {
        ...
        store.dispatch('update_rrhh_state', data).then(res => {
  
          if(res.data.estado == true){
  
            this.$store.commit({
              type  : 'toggleEstadoRrHh',
              id    : data.codProyecto,
              estado: data.state
            });
      ...
      selectUserFunc: function(payload) {
        ...
        // this.currentRes = this.$store.state.restriction_rows[payload-1]
          ...
          let datos          = buscar.map((id) => (this.$store.state.rrhh_rows.find(x => x.codProyecto == id)));
      ...
      async createRRHHForProjects(){
        try{
          const response = await axiosClient.post('/get_all_rrhh');
        } catch (error){
    ...
    computed: {
      rows: function() {
       ...
       //store.dispatch('get_restrictions')
        return this.$store.state.rrhh_rows;
      ...
      estadoRestriccion: function() {
         return this.$store.getters.statusRrHh(this.rowId);
      },
      isLoadingTrue(){
         return true
      },
